...
\begin{document}
    ...
    \begin{twocolentry}{
        ...
        \textbf{카카오페이}, {\small Backend Software Engineer}
        \begin{highlights}

            \item \textbf{\small 테크핀 도메인 역량} : {\small 충전, 송금, 신한은행 등 외부사와 금융 제휴 상품 개발}
            \item \textbf{\small 무결성 시스템 개발 역량} :
            \begin{highlights}
                \item {\small 금액 무결성을 위한 대사 작업, 재처리 프로세스, 동시성 관리, 멱등적 API 설계}
            \end{highlights}
            \item \textbf{\small 협업 역량} : {\small 페어프로그래밍, 스프린트, 이벤트 스토밍}
        \end{highlights}
    \end{twocolentry}
    ...
    \begin{twocolentry}{
        ...
        \textbf{뱅크샐러드}, {\small Backend Software Engineer}
        \begin{highlights}

            \item {\small 금융사와 PLCC 카드 개발, Open API 통신 개발}
        \end{highlights}
    \end{twocolentry}
    ...
    \begin{twocolentry}{
        ...
        \begin{highlights}
            ...
            \item \textbf{역할} :
            \begin{highlights}
                \item UTC+1 차이를 고려한 안정적인 대사 및 개발 진행
    ...
    \begin{twocolentry}{
        ...
        \begin{highlights}
            ...
            \item \textbf{성과} : 적금 계좌 20,000건 이상 개설
            \item \textbf{역할} :
            \begin{highlights}
                \item 적금 계좌로 송금 기능 개발과 잔액 정합성을 위한 재처리 프로세스 및 대사 개발
                \item 은행 지연 대비 서킷브레이커 적용
                \item PM, 개발자의 동일한 컨텍스트 공유를 위한 스프린트 및 이벤트 스토밍 문화 도입
            \end{highlights}
        \end{highlights}
    \end{twocolentry}
    ...
    \begin{twocolentry}{
        ...
        \textbf{마이데이터 계좌 다건을 페이에 한번에 연결하는 프로젝트}
        \begin{highlights}

            \item \textbf{설명} : 기존 1건 계좌 연결을 개선해 평균 5건 이상 은행 계좌 한번에 연결하는 프로젝트
            \item \textbf{성과} : 직전 월 대비 평균 계좌 연결 건수 85\% 증가 (54만건 -> 100만건)
            \item \textbf{역할} :
            \begin{highlights}
                \item webflux 병렬성 이용하여 일반 동기호출 대비 레이턴시 3초 -> 0.5초로 개선. (평균 10건 기준)
                \item 계좌 연결할 때 타임아웃 재시도를 위해 카프카 retry topic 및 dlt 활용
            \end{highlights}
        \end{highlights}
    \end{twocolentry}
    ...
    \begin{twocolentry}{
        ...
        \begin{highlights}
            ...
            \begin{highlights}
                ...
                \item 코드리뷰 비용을 줄이고 개발 팀원 모두가 동일한 컨텍스트를 공유하기 위해 페어프로그래밍 진행
            \end{highlights}
        \end{highlights}
    \end{twocolentry}
    ...
    \begin{twocolentry}{
        ...
        \begin{highlights}
            ...
            \item \textbf{설명} : 평균 초당 13건 발생하는 미수금을 위한 충전 기능 제공하는 프로젝트
            \item \textbf{성과} : 잔액 정합성 유지하며 45,000건/시간, 13TPS 트래픽 처리, 약 13억/일 충전금액 증대 (peak 기준)
            \item \textbf{역할} :
            \begin{highlights}
                \item 잔액 정합성을 위해 레디스를 이용한 분산락을 사용하여 동시성 관리 % TODO : 내용 반복되어서 추가하기
    ...
    회사에서 팀원들이 불편함을 겪는 사항을 자발적으로 개선하기 위한 프로젝트 입니다.

    \begin{highlights}

        \item 코드리뷰 지연 방지를 위한 PR 알림 봇
        \begin{highlights}
            \item MSA로 프로젝트가 많아지면서 리뷰가 늦어지는 경우 발생
            \item 매일 아침 approved 받지 못해 리뷰가 필요한 pr 모아서 슬랙 알림
            \item 결과 : 평균 20건 -> 7건
        \end{highlights}
    ...
    \begin{highlights}
        \item \textbf{Framework \&& Library:} Spring Boot, Spring Batch, JPA, Gradle, Circuit breaker
    \end{highlights}
